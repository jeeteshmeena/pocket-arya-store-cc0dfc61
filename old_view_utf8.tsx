            ...
            {activeTab === "dashboard" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="bg-[#1a1a1a] text-white p-6 rounded-[28px] shadow-xl relative overflow-hidden">
                  ...
                  <div className="text-4xl font-black tracking-tight mb-6 relative z-10">Γé╣{stats?.total_revenue?.toLocaleString() || 0}</div>
                  <div className="grid grid-cols-2 gap-4 relative z-10">
                    <div className="bg-white/10 rounded-2xl p-3 backdrop-blur-md">
                      ...
                      <div className="font-bold">Γé╣{buyers?.reduce((s:number,b:any)=>s+(b.amount||0),0).toLocaleString() || 0}</div>
                    </div>
                    <div className="bg-white/10 rounded-2xl p-3 backdrop-blur-md">
                    ...
                    {stats?.recent_orders?.slice(0,3).map((o: any, i: number) => (
                      <div key={i} className="bg-white p-4 rounded-[20px] shadow-sm border border-gray-100 flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <div className="font-bold text-sm">Γé╣{o.total_amount || o.amount || 0}</div>
                      ...
                      {stories.map(story => (
                        <div key={story.story_id} className="p-4 rounded-[20px] bg-white border border-gray-100 shadow-sm flex gap-4 items-center">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-500 mt-0.5">Γé╣{story.price} ΓÇó {story.status}</div>
                          </div>
                    ...
                    {buyers.map((buyer, i) => (
                      <div key={i} className="bg-white p-5 rounded-[24px] shadow-sm border border-gray-100">
                        <div className="flex justify-between items-start mb-4">
                          ...
                          <div className="text-right">
                             <div className="font-bold text-emerald-500">Γé╣{buyer.amount}</div>
                          ...
                          <div className="bg-gray-50 rounded-xl p-3 mb-4">
                            {buyer.payments.map((p:any, j:number) => (
                               <div key={j} className="flex justify-between text-xs py-1 border-b border-gray-200 last:border-0">
                                  ...
                                  <div className="text-right font-bold">Γé╣{p.amount}</div>
                               </div>
                            ))}
                          </div>
                        )}
          ...
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-t-[32px] sm:rounded-[32px] p-6 bg-white shadow-2xl relative">
            ...
            <form onSubmit={handleSave} className="space-y-5">
              <div className="space-y-4 p-5 rounded-[24px] bg-gray-50 border border-gray-100">
                ...
                <div className="flex gap-2">
                  <button type="button" disabled={!!translating} onClick={() => handleAutoTranslate("hi")} className="flex-1 py-2.5 text-xs font-bold rounded-[12px] bg-blue-50 text-blue-600 disabled:opacity-50">EN ΓåÆ HI</button>
                  <button type="button" disabled={!!translating} onClick={() => handleAutoTranslate("en")} className="flex-1 py-2.5 text-xs font-bold rounded-[12px] bg-emerald-50 text-emerald-600 disabled:opacity-50">HI ΓåÆ EN</button>
                </div>
                <div>
              ...
              <div className="space-y-4 p-5 rounded-[24px] bg-gray-50 border border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1 block">Price (Γé╣)</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-1 block">Discount (Γé╣)</label>
