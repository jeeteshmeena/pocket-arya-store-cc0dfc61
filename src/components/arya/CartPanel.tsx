...
export function CartPanel() {
  const { cartOpen, setCartOpen, cart, removeFromCart, startCheckout } = useApp();
      ...
      <aside className="absolute right-0 top-0 h-full w-[88%] max-w-sm bg-background border-l border-border flex flex-col animate-slide-in-right">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 space-y-3">
          ...
          {cart.map((s) => (
            <div key={s.id} className="flex gap-3 p-2 rounded-xl bg-surface">
              ...
              <button onClick={() => removeFromCart(s.id)} className="h-9 w-9 grid place-items-center rounded-full hover:bg-background text-muted-foreground">
        ...
        {cart.length > 0 && (
          <div className="border-t border-border p-4 space-y-3">
            ...
            <button
              onClick={() => { setCartOpen(false); startCheckout(cart); }}
